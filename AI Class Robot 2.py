...
def program(percepts):
    ...
    for direction_list, action_up, action_down in [
        (ts_up, 'cleanup', 'cleandown'),
        (ts_down, 'cleandown', 'cleanup'),
        (ts_left, 'cleanleft', 'cleanright'),
        (ts_right, 'cleanright', 'cleanleft'),
        (ts_up_left, 'cleanupleft', 'cleandownright'),
        (ts_up_right, 'cleanupright', 'cleandownleft'),
        (ts_down_left, 'cleandownleft', 'cleanupright'),
        (ts_down_right, 'cleandownright', 'cleanupleft')
    ]:
        for t in direction_list:
            if isinstance(t, Chair) or isinstance(t, Trolley):
                if not bot.needed_cleaning:
                    bot.needed_cleaning.extend([action_up, action_down])

    if bot.needed_cleaning:
        print(bot.needed_cleaning)
